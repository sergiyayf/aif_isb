...
def plot_width_distributions(ax = None, n_radii=10, kernel='logGauss'):
    ...
    pulled_b = w.pull_histories_together('random_switching_new', clone_type='blue')
    if kernel == 'compare':
        start_range = 60
        end_range = 65
    else:
    ...
    for idx, loc in enumerate(range(start_range, end_range, 10)):
        ...
        colors = [plt.get_cmap('Blues')(x) for x in np.linspace(0.1, 1, n_radii)]
...
def setup_figure(save_figure=False, compare=False):
    ...
    if save_figure:
        fig.savefig(r'SI_Rev_Width_distribution_blue_logKDE_pts.pdf', transparent=True)
