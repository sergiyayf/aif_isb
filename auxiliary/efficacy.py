...
def plot_efficacy(ax, CHX, BED, treatment, color='r', label='a set', **kwargs):
    """
    plot efficacy
    ...
    no_switch_n = colplt.get_numbers(50, 0, treatment, 0) + colplt.get_numbers(50, 0, treatment,
                                                                               1) + colplt.get_numbers(50, 0, treatment,
                                                                                                       2)
    no_switch = no_switch_n / no_switch_n[0]

    switch_n = colplt.get_numbers(CHX, BED, treatment, 0) + colplt.get_numbers(CHX, BED, treatment,
                                                                               1) + colplt.get_numbers(CHX, BED,
                                                                                                       treatment, 2)
    switch = switch_n / switch_n[0]
    # print(no_switch/switch)
    # print(np.sqrt( (error(switch_n,switch_n[0])/switch)**2 + (error(no_switch_n,no_switch_n[0])/no_switch)**2 ))
    # print(no_switch/switch*np.sqrt( (error(switch_n,switch_n[0])/switch)**2 + (error(no_switch_n,no_switch_n[0])/no_switch)**2 ))
    err = no_switch / switch * np.sqrt(
        (error(switch_n, switch_n[0]) / switch) ** 2 + (error(no_switch_n, no_switch_n[0]) / no_switch) ** 2)
    # err = np.sqrt(err**2+Rad_errs**2)
    ax.plot(Rad, 1 - no_switch / switch, '-o', linewidth=1., color=color, label=label, **kwargs)
    ax.fill_between(Rad, 1 - no_switch / switch - err, 1 - no_switch / switch + err, linewidth=1., color=color,
                    alpha=0.5)

    # plt.errorbar(radius(condition), 1-no_switch/switch,yerr = err, linewidth=3., color = color, label = label )
...
def plot_efficacy_only_points(ax, CHX, BED, treatment,filename = 'experimental_data.h5', color='r', label='a set'):
    """
    plot efficacy
    ...
    ax.errorbar(new_r_no_switch, 1 - no_switch / switch,yerr=err, fmt='s',ms = 3, linewidth=1., color=color, label=label)
...
if __name__ == '__main__':
    ...
    plot_efficacy(ax,50,4,10,color='k',ms = 1)
