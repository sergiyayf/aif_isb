...
%structure mat file
function switch_analyze(input_filename, input_directory, Colony_output_directory, Colony_output_name, Image_output_directory)
...
A = imread(fullfile(dir, file)); % read image that is segmented in Ilastic
...
%label)
[max_x_black, index_max] = max(row3); 
y_pos_max = col3(index_max);

[min_x_black, index_min] = min(row3); 
y_pos_min = col3(index_min);
...
[xx0,yy0,RR] = circle_fit(row3,col3); 
c = [xx0,yy0];
...
freq_red = red_pixels/length(theta); 
freq_blue = blue_pixels/length(theta); 

Colony(i) = get_data_switch(file,RR, rad_microns, center, Streaks_matrix, segment_color, count_red, count_blue, count_mixed, count_total, freq_red, freq_blue);
