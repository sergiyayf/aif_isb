function [Cell_CHX, Cell_BED, Cell_CHX_JK, Cell_BED_JK] = switch_conditions_separate(look_at_day) 
CHX=[0 25 50 100 150];
...
dataCol = [];
dataCol_JK = [];
load(['data_switch_' num2str(day_to_look) '.mat'],'Colony');
dataCol = Colony; 
load(['data_switch_' num2str(day_to_look) '_JK.mat'],'Colony');
...
for j = 1:1:Sn
      ...
      rad_microns = [rad_microns dataCol(j).radius_microns];
end
...
for i = 1:1:length(inds)
    ...
    for j = 1:1:length(index)
       ...
       red_freq = [red_freq; dataCol(k).red_frequency];
       blue_freq = [blue_freq; dataCol(k).blue_frequency];
    end
...
for j = 1:1:SJK
      ...
      rad_microns = [rad_microns dataCol_JK(j).radius_microns];
end
...
for i = 1:1:length(inds)
    ...
    for j = 1:1:length(index)
       ...
       red_freq = [red_freq; dataCol_JK(k).red_frequency];
       blue_freq = [blue_freq; dataCol_JK(k).blue_frequency];
    end
