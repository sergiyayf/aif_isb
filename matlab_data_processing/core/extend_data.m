...
dataCol = [];
for day=DAYS
        load(['data_experiment_' num2str(day) '.mat'],'Colony');
        dataCol = Colony;
...
for j = 1:1:Sn(2)
      ...
      red_freq = sum(dataCol(j).streaks.size(strk_red_ind))/(2*pi); 
      blue_freq = sum(dataCol(j).streaks.size(strk_blue_ind))/(2*pi); 
      tot_freq = sum(dataCol(j).streaks.size)/(2*pi);
      %dataCol(j).red_freq = red_freq; 
      %dataCol(j).blue_freq = blue_freq; 
...
dataCol_JK = [];
for day=DAYS_JK
        load(['data_experiment_' num2str(day) '_JK.mat'],'Colony');
        dataCol_JK = Colony;
...
for j = 1:1:Sn(2)
      ...
      [strk_red, strk_red_ind] = find(Color == 0); 
      [strk_blue, strk_blue_ind] = find(Color ==1); 
      red_freq = sum(dataCol_JK(j).streaks.size(strk_red_ind))/(2*pi); 
      blue_freq = sum(dataCol_JK(j).streaks.size(strk_blue_ind))/(2*pi); 
      tot_freq = sum(dataCol_JK(j).streaks.size)/(2*pi);
     % dataCol_JK(j).red_freq = red_freq; 
      %dataCol_JK(j).blue_freq = blue_freq; 
