...
[filename, directory] = uigetfile([pwd '\..\..\main_tiffs\Segmentation\*.tif'], 'Select at least 2 segmented tif files', 'MultiSelect','on');  %open the file to process by choosing it
...
for alpha = start_angle:pi/720:stop_angle
    ...
    if label_now ~= label_old 
        ...
        elseif (label_now == 1 | label_now == 5 | label_old == 4) & label_old == 2 % blue -> yellow 
        ...
        elseif (label_now == 1 | label_now ==5 | label_old == 4) & label_old == 3 % red -> yellow 
                    is_not = is_not_a_sector(A,xx,yy,20);
                    if is_not == false
                    segment_stop_alpha = [segment_stop_alpha; alpha];
                    lbl = lbl+1;
